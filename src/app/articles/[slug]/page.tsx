...
const placeholders: Record<string, { title: string; topic: string; slug: string; content: ContentBlock[]; tags: string[]; excerpt: string; publishedAt: string; readingTime: number }> = {
...
export default async function ArticleAlternatePage(props: { params: Promise<{ slug: string }> }) {
  ...
  const tocItems = article.content
    .filter((b: ContentBlock) => b._type === 'block' && ['h2', 'h3'].includes(b.style || ''))
    .map((b: ContentBlock) => {
      const text = b.children?.map((c: { text: string }) => c.text).join('') || b.children?.[0]?.text || ''
      return {
        id: slugify(text),
        text,
